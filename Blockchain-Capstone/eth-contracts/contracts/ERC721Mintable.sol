...
contract Ownable {
    ...
    constructor() public {
        _owner = msg.sender;
        emit TransferOfOwnership(_owner);
    }
...
contract Pausable is Ownable {
    ...
    function setPaused(bool status) public onlyOwner {
        require(
            paused != status,
            "Specified pause state must be different than current pause state"
        );
        paused = status;
        if (paused) {
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(
            ...
            "To address can't be  same as token owner"
        );
        require(
    ...
    function _transferFrom(
        ...
        require(
            ...
            "Address is not the owner of the token"
        );
